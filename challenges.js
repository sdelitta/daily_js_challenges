...
function reverseUpcaseString(string) {
  ...
  for (i = 0; i < string.length; i++){
    reverse = string.charAt(i).toUpperCase + reverse
  }
return reverse
}
/*-----------------------------------------------------------------
...
function isPrime(n) {
  ...
  for (let int = 2; int <= int / 2; int++) {
    if (Number.isInteger(n / int)) return false
  }
  return true
}
/*-----------------------------------------------------------------
...
function balancedBrackets(str) {
  let arr = [];
  return str.split('').every(c => '([{'.includes(c) ? arr.push(c) : '() {} []'.includes(arr.pop() + c));
}
/*-----------------------------------------------------------------
